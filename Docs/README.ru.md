...
<details>
<summary><b>Система Тем [Подробное Руководство](v2.0.9613_themes_en.md)</b></summary>
...
<details open>
<summary><b>Изменения в v2.0.9623</b></summary>

## Изменения в v2.0.9623

### Проверка и Применение Обновлений — В Стиле Лаунчера, Вручную, Без Блокировки

Всплывающее окно первого запуска (окно `FirstSetup`) раньше предлагало 3 опции — **Сохранять Последнюю Версию**, **Поиск Обновлений** и **Подключение к Steam** — разработка которых была приостановлена. Изучение реального состояния кода перед возобновлением работы показало, что Подключение к Steam и Сохранять Последнюю Версию уже полностью работали; незавершённым был только Поиск Обновлений (проверка наличия более новой версии ModAPI) — конвейер загрузки/установки существовал, но его никто никогда не вызывал.

**Что было реализовано:**

- **`Game.CheckForNewVersion(out releaseNotes)`** (`ModAPI_Shared\Data\Game.cs`) — вызывает GitHub Releases API (`/repos/{owner}/ModAPI/releases/latest`) и возвращает последний тег вместе с заметками о релизе (поле `body`, разобранное минимальным JSON-строковым разэкранировщиком — без добавления новых зависимостей). `UpdateRepoOwner` для всех обычных пользователей всегда указывает на `FluffyFishGames` и переключается на репозиторий разработки мейнтейнера только при запуске с `--dev` — никогда на основании галочки "Лог разработчика" на вкладке Настройки, поскольку эта галочка предназначена исключительно для детализации логов сбоев и не должна незаметно переводить пользователя на неопубликованный тестовый канал.
- **Вкладка Настройки**: нерабочая галочка "Поиск Обновлений" исчезла, заменена одной кнопкой **Обновить** (в стиле лаунчера, а не фоновым переключателем). Нажатие проверяет наличие новой версии и, если она есть, сразу запускает процесс загрузки/распаковки/применения — без промежуточного выбора "сейчас или позже".
- **Окно первого запуска**: мёртвая опция "Автообновление" удалена вместе с уже неиспользуемой записью в конфигурацию; окно теперь спрашивает только про Подключение к Steam и Сохранять Последнюю Версию.
- **Окно `OperationPending`** получило необязательную, свёрнутую по умолчанию панель заметок о релизе и событие `Confirmed`. Когда загрузка/распаковка достигает 100%, заметки о релизе становятся видимыми, и кнопка "Готово" активируется — именно её нажатие фактически запускает `Updater.exe` и закрывает ModAPI (раньше это происходило автоматически сразу по завершении распаковки, без возможности увидеть, что изменилось). Сам `Updater.exe` (код оригинального автора, без изменений) ждёт завершения работы ModAPI, перезаписывает файлы и автоматически перезапускает ModAPI.
- ModAPI никогда не блокируется из-за устаревшей версии: проверка версии выполняется только при нажатии пользователем кнопки Обновить, никогда при запуске — приложение остаётся полностью работоспособным независимо от статуса обновления, в отличие от типичного принудительного шлюза обновлений игрового лаунчера.

### Исправление UI — Панель Заметок о Релизе Не Переносила Текст

Новая панель заметок о релизе в окне прогресса `OperationPending` изначально отображалась как одна нечитаемая строка, требующая горизонтальной прокрутки, по двум наложившимся друг на друга причинам:

1. Значение `HorizontalScrollBarVisibility` у `TextBox` по умолчанию — `Hidden`, а не `Disabled` — и пока оно явно не установлено в `Disabled`, WPF игнорирует `TextWrapping="Wrap"` и позволяет строке расти бесконечно.
2. Стиль `SubWindow`, используемый всеми всплывающими окнами, задаёт `SizeToContent="WidthAndHeight"` (`FluentStyles.xaml`), поэтому панель, растягивающаяся через колонку сетки `*`, не имеет фиксированной ширины для переноса текста — ей нужно явно задать ширину.

Исправлено путём задания панели фиксированной ширины, привязанной к `ProgressBar.ActualWidth` (чтобы её правый край выравнивался с полосой прогресса выше), и заменой `TextBox` на доступный только для чтения `TextBlock` внутри `ScrollViewer` (`TextBlock` не имеет особенности переноса `PART_ContentHost`, присущей `TextBox`). Панели также задан непрозрачный фон `FluentCardBrush` — ранее она по умолчанию наследовала прозрачность окна. Ручная корректировка `Height +=`, добавленная ранее для принудительного увеличения высоты окна, была удалена, поскольку она активно конфликтовала с `SizeToContent` — теперь окно корректно увеличивается само, как только панель становится видимой.

### Проверено от Начала до Конца (Ручное Тестирование)

Полный цикл проверка → загрузка → распаковка → подтверждение → перезапуск был реально пройден (на одноразовой тестовой полезной нагрузке релиза), чтобы подтвердить работоспособность всей цепочки, включая оригинальный неизменённый `Updater.exe`. Одна полезная вещь, выясненная в ходе тестирования: `App.xaml.cs` уже удаляет оставшуюся папку `_update` при собственном запуске ModAPI (в качестве шага самоочистки) — поэтому любой ручной тест, заранее создающий поддельную папку `_update` *до* запуска ModAPI, приведёт к тому, что она будет незаметно поглощена ещё до того, как `Updater.exe` вообще её увидит. Папку нужно создавать *пока ModAPI уже запущен*, так же как это делает реальная загрузка. В `Updater.cs` добавлено необязательное диагностическое логирование (`Updater.diag.log`, обёрнутое в try/catch, вне обычного пути логики), чтобы в будущем было проще диагностировать подобные случаи без необходимости трогать саму оригинальную логику перезапуска.

### Заметки для Разработчиков — Запуск с `--dev`

`App.DevMode` (`ModAPI\App.xaml.cs`) становится `true` только при запуске приложения с аргументом командной строки `--dev` — это не связано с галочкой "Лог разработчика" на вкладке Настройки, которая влияет только на детализацию логов и **не должна** использоваться для управления чем-либо ещё, кроме логирования (например, она никогда не должна переключать, к какому серверу/репозиторию обращается функция — пользователь, включивший подробное логирование для сообщения о сбое, не должен быть незаметно переведён на неопубликованный тестовый канал).

Чтобы запустить локально с `--dev`:
- **Visual Studio (отладка F5)**: проект `ModAPI` → Свойства → вкладка Отладка → "Аргументы командной строки" → ввести `--dev`.
- **Собранный .exe**: запустить из терминала как `ModAPI.exe --dev`, либо добавить `--dev` в конец поля Объект ярлыка.

### "Держать Таблицу Версий" — Теперь Отключена, Пока От Неё Есть Реальная Польза

`Game.Verify()` доходит до `VersionsData.Refresh()` (кода, который действительно загружает таблицу версий) только если путь к игре корректен (`CheckGamePath()` проходит) — в противном случае он завершается раньше. Это означало, что включение «Держать Таблицу Версий» без настроенного пути к игре молча ничего не делало, что вызывало путаницу (подтверждено на практике: ручной тест без пути к игре не дал ни одной строки лога `[UpdateVersions]`, даже при включённой галочке).

Добавлено `SettingsViewModel.CanUpdateVersionsTable` (`App.Game != null && App.Game.CheckGamePath()`) и привязано к `IsEnabled` галочки. Значение пересчитывается при каждом сохранении/сбросе пути к игре или переключении игры в фильтре вкладки Development. Когда галочка отключена, наведение показывает подсказку с объяснением причины; когда включена — подсказку о том, что даёт включение. Также корейская подпись переименована с «최신버전 유지» («сохранять последнюю версию») на «버전 테이블 유지» («держать таблицу версий») — на всех остальных языках здесь уже было слово «таблица», и только в корейском оно отсутствовало, что путало эту опцию с не связанной с ней кнопкой «Обновить».

### Глобальное Исправление — У Подсказок Не Было Стиля в Теме По Умолчанию (classic)

Подсказка из предыдущего раздела отображалась как обычная системная белая подсказка вместо оформленного вида приложения. Выяснилось, что ни одна подсказка в приложении никогда раньше не проверялась в теме «classic» по умолчанию: файлы `FluentStyles*.xaml` (темы, отличные от базовой) каждый определяют оформленный стиль `ToolTip`, но `classic` загружает только `Dictionary.xaml`, где такого стиля никогда не было. В `Dictionary.xaml` добавлен соответствующий стиль `ToolTip` (непрозрачный фон, поскольку остальные полупрозрачные кисти карточек этой темы рассчитаны на фон-изображение, а не на плавающую подсказку). Это исправляет подсказки во всём приложении под темой по умолчанию, а не только в этом конкретном месте.

### Переработка Окна Первого Запуска

Окно первого запуска больше вообще не спрашивает про Подключение к Steam / Держать Таблицу Версий — обе настройки уже есть на вкладке Settings, поэтому повторный вопрос здесь был избыточен. Вместо них окно теперь показывает прокручиваемую сводку «Что нового в этой версии». Вступительный текст и кнопка также переработаны:

- Галочки Подключения к Steam / Держать Таблицу Версий и их описания убраны; на их месте — прокручиваемая панель фиксированной ширины с обзором изменений релиза.
- Заголовок «환영합니다!» («Добро пожаловать!») на вкладке Welcome заменён кнопкой с тем же названием — нажатие на неё в любой момент повторно открывает окно первого запуска, например чтобы перечитать список изменений текущей версии. При открытии таким образом кнопка окна называется «Закрыть» вместо «Продолжить», не выполняет повторно первоначальную настройку (нет записи `SetupDone`, нет вызова `FirstSetupDone()`), и закрытие окна никогда не завершает работу приложения (`isReopen: true` в конструкторе `FirstSetup` управляет всем этим поведением).
- **Ошибка размера окна, найденная при кросс-тематическом тестировании**: стиль `SubWindow` этого окна сочетает `AllowsTransparency="True"` + `WindowStyle="None"` + `SizeToContent="WidthAndHeight"` — сочетание, при котором WPF не всегда надёжно учитывает `MaxWidth` при авторазмере. Под темой `classic` это случайно выглядело нормально, но под другими темами вроде Diablo окно рендерилось слишком широким (с непереносимым, обрезанным текстом). Исправлено переопределением `SizeToContent="Height"` локально для этого окна (карточки в этом окне по замыслу имеют фиксированную ширину, поэтому автоподбор ширины никогда и не был нужен) — это одинаково исправляет раскладку во всех темах вместо патча под каждую тему по отдельности.
- **Читаемость, только тема `classic`**: общий стиль `NormalLabel` темы `classic` (белый текст с тенью, рассчитанный на текст поверх фотографических фоновых панелей) плохо читается на сплошном фоне карточки `PanelCenter` этого окна. Вместо изменения общего стиля (используемого повсеместно) или жёсткого задания цветов, которые смотрелись бы неуместно в любой другой теме, добавлен `FirstSetup.ApplyClassicThemeTextFix()`, строго привязанный к условию `App.GetCurrentTheme() == "classic"`, который подменяет вид на тёмный, без тени, только для этого окна и только под этой одной темой. Все остальные темы не затронуты и продолжают использовать свои уже корректные цвета `NormalLabel`/`PanelCenter`.

### Проверка Целостности Игры — Шаг C Больше Не Спрашивает При Каждом Запуске

Пользователи сообщали о реальном раздражении из-за предзапусковой проверки целостности: для игр, которые попросту распространяются без цифровой подписи (обычное дело для инди-тайтлов вроде Green Hell), предупреждение «нет подписи» появлялось **при каждом без исключения** нажатии Start Game, требуя ручного клика «Продолжить» каждый раз — даже когда на самом деле всё было в порядке. Само по себе отсутствие подписи не доказывает вмешательство, так что это было чистое раздражение, а не реальная проверка безопасности.

```mermaid
flowchart LR
    Start(["Нажатие Start Game"]) --> A{"A — заголовок PE\nIsValidGameExe()"}
    A -- ошибка --> ABlock["🛑 Блокировка запуска\nвсплывающее окно GameExeCorrupted"]
    A -- успех --> B{"B — контрольная сумма сборки\nMD5 vs Versions.xml"}
    B -- несоответствие --> BBlock["🛑 Блокировка запуска\nвсплывающее окно GameAssemblyTampered"]
    B -- совпадение --> C{"C — цифровая подпись\nHasDigitalSignature()"}
    C -- отсутствует --> CLog["📝 Только запись в лог, без окна\nпродолжает автоматически"]
    C -- присутствует --> CLog2["📝 Только запись в лог"]
    CLog --> Launch(["✅ Игра запускается"])
    CLog2 --> Launch
```

- **A (заголовок PE)** и **B (контрольная сумма сборки)** не изменились — реальное повреждение или вмешательство по-прежнему блокирует запуск всплывающим предупреждением (`NoProjectWarning`, оформленным через общий стиль `SubWindow`, как и любое другое окно в приложении, поэтому оно автоматически соответствует активной теме).
- **C (цифровая подпись)** больше не показывает никакого окна и не запрашивает подтверждение в любую сторону — просто записывает строку уровня `Notice` (`[Integrity] Game executable has no digital signature (not necessarily tampered — many games ship unsigned)`) и даёт игре запуститься. Класс окна `GameIntegrityWarning`, который раньше для этого открывался, больше нигде не вызывается (оставлен в коде неиспользуемым, а не удалён).
- Эта диаграмма нужна для того, чтобы форму проверки было легко обсуждать позже — например, если шаг C когда-нибудь потребуется вернуть в более лёгкой форме (вариант «больше не спрашивать» вместо полного удаления запроса рассматривался и был отклонён в пользу полного удаления, по указанию пользователя), диаграмма выше служит опорной точкой.

### Подключение к Steam — Автоопределяемый Путь и Ошибка Ручного Редактирования

Проверка того, что на самом деле делала функция «Подключение к Steam» оригинального автора (помимо выбора пользователем пути к Steam), показала, что она также запускает игру через `Steam.exe -applaunch {AppId}` (для поддержки оверлея) и восстанавливает повреждённые файлы через `steam://validate/{AppId}` — обе возможности уже реализованы и не тронуты в этом раунде.

- В момент включения «Подключение к Steam» `MainWindow.UseSteamCheckBox_Checked` считывает путь к Steam прямо из реестра (`HKEY_CURRENT_USER\Software\Valve\Steam`) и автоматически заполняет поле — это работает независимо от того, на каком диске установлен Steam, а не только на `C:`.
- Пока «Подключение к Steam» включено, элементы ручного управления путём (текстовое поле, Browse, Save, Reset) должны быть отключены, поскольку путь управляется автоматически. **Найдена и исправлена ошибка**: у Grid-контейнера, содержащего эти элементы (`SteamAndGamePathsPanel`), в коде-behind никогда не устанавливался `DataContext` — только у `Settings` и `SettingsCheckboxes` — из-за чего `{Binding CanEditSteamPathManually}` молча не срабатывал и по умолчанию оставался `IsEnabled="true"`. Элементы *выглядели* неактивными, но кнопка Reset оставалась полностью кликабельной. Исправлено явной установкой `SteamAndGamePathsPanel.DataContext = SettingsVm;` наряду с двумя другими.
- Отдельно выяснилось, что отключённые элементы управления по всему приложению вообще не давали визуальной обратной связи под темой classic — у `ControlTemplate` стиля `NormalButton` не было триггера `IsEnabled="False"` (у Fluent-тем он уже был). Добавлен аналогичный триггер, затемняющий кнопку до 40% непрозрачности при отключении, по всему приложению под темой classic.

### Унификация Системы Тем — Паритет Classic ↔ Fluent

По вопросу «зачем вообще держать classic отдельно от семейства тем Fluent, если они не так уж и отличаются» проведён полный аудит, сравнивающий каждый явный и неявный стиль между `Dictionary.xaml` (classic) и 9 файлами `FluentStyles*.xaml`.

- Найдены реальные, действующие пробелы: `Slider` (используется ползунками ширины списка модов/проектов на вкладке Settings) и `ComponentsInputs:MultilingualTextField` (сочетание языкового флага и текстового поля для полей названия/описания мода) существовали только в classic, без аналога во Fluent — под темой Fluent эти два элемента молча откатывались к Scale9-текстурному оформлению classic, ломая плоский вид Fluent. Для обоих построены плоские замены на `DynamicResource` и помещены в один новый общий файл, **`ModAPI\Themes\FluentStylesShared.xaml`**, подключённый ко всем 9 файлам `FluentStyles*.xaml` через `ResourceDictionary.MergedDictionaries` — теперь правку цвета нужно вносить только в одном месте вместо девяти.
- Найден и обратный пробел: у `GridSplitter` (разделитель списка модов/версий в `MainWindow.xaml`) был стиль Fluent во всех 9 темах, но не было в classic, из-за чего под classic он рендерился обычным серым сплиттером по умолчанию ОС. В `Dictionary.xaml` добавлен соответствующий плоский стиль.
- По ходу дела найден действительно мёртвый код — стили, определённые, но нигде не используемые в реальном интерфейсе: `PasswordBox` (используется только в `LoginWindow.xaml`, который сам нигде не создаётся — система логина уже была удалена в v2.0.9400), `Components:ModProjectButton`, четыре стиля кнопок соцвходов (`FacebookButton`/`TwitterButton`/`YoutubeButton`/`TwitchButton`) и набор `TimeSlider`/`TimeHorizontalSlider`/`TimeSliderThumbStyle` (вероятно, ползунок дня/ночи, который так и не вышел в релиз). Следуя позиции проекта «не удалять работу оригинального автора», ничего из этого не удалено — каждый блок обёрнут в XML-комментарий с пометкой о причине неиспользования, чтобы он оставался в файле как запись истории, а не исчезал бесследно.
- Найдены два файла, оставшихся вне сборки вообще: `ModAPI\Windows\Dictionary.xaml` (тупиковый дубликат, созданный в середине рефакторинга в раннем коммите, никогда не упоминаемый ни `App.xaml`, ни `.csproj`) и `ModAPI\Themes\FluentStylesClassic.xaml` (ранний прототип системы тем — изначально служивший резервным оформлением для всех тем, кроме `light`, ещё до того, как у каждой темы появился собственный отдельный файл; стал бесхозным после замены этой резервной логики). Оба подтверждены по истории git как не связанные с кодом оригинального автора, поэтому — в отличие от мёртвых стилей выше — они были удалены полностью, а не закомментированы, вместе с удалением ставшей висячей записи `<Page>` для `FluentStylesClassic.xaml` в `.csproj`.
- Собственный `Slider` темы classic затем переработан под плоский вид общего стиля Fluent (та же структура `Border`+`Track`, перекрашенная в золотисто-коричневую палитру classic: ползунок `#B8963E`, полупрозрачная дорожка `#40FFFFFF`) вместо прежней полосы на Scale9-текстуре — старая реализация (`SliderThumbStyle`, `SliderButtonStyle`, `HorizontalSlider`, `VerticalSlider` и старый неявный стиль `Slider`) аналогично закомментирована, а не удалена.

### Подсказки ВКЛ/ВЫКЛ для Остальных Галочек Вкладки Settings

Тот же паттерн «наведи, чтобы узнать, что это делает», применённый ранее к «Держать Таблицу Версий», распространён на остальные четыре галочки вкладки Settings — **Подключение к Steam**, **Лог разработчика**, **Очищать логи при запуске** и **Всегда поверх окон** — каждая теперь показывает разную подсказку в зависимости от собственного состояния, объясняя, что именно даёт включение и выключение (например, подсказка Подключения к Steam в включённом состоянии объясняет автоопределение пути и поддержку оверлея, описанные выше). 8 новых языковых ключей × 13 языков.

### Окно Приветствия — Полное Детальное Содержимое, Синхронизированное с Заметками о Релизе

Панель «Что нового в этой версии» прошла через несколько итераций в этом раунде: краткая сводка по пунктам → полный, по-разделам, обзор, соответствующий `Docs/RELEASE_NOTES_2.0.9623.md` (с обычными текстовыми маркерами `■`/`▸`/`•`, поскольку `TextBlock` не умеет рендерить Markdown), переведённый на все 13 языков. По ходу дела всплыли и были исправлены две связанные ошибки раскладки:

- У прокручиваемого текста изначально была жёстко заданная `Width="450"`, оставлявшая зазор перед полосой прокрутки (а в теме classic ещё и переносившая текст слишком рано). Исправлено удалением фиксированной ширины и заданием `ScrollViewer` отступа `Padding="14"` — тот же приём, уже проверенный в панели деталей `OperationPending`.
- Окружающей рамке (`WhatsNewBorder`) затем потребовалась ширина, и жёстко заданное значение в пикселях оказалось проигрышной затеей: собственные внутренние отступы шаблона `SubWindow` различаются по темам (32px суммарно под classic против 72px под Fluent, поскольку шаблон Fluent добавляет и внешний отступ `Border`, и отступ `ContentPresenter`). Ширина, подобранная так, чтобы точно заполнить более просторную раскладку classic, **обрезала полосу прокрутки за пределы видимого края** под темами Fluent. Исправлено полным удалением явной ширины и `HorizontalAlignment="Left"` — теперь рамка по умолчанию растягивается (`Stretch`) и корректно заполняет всё пространство, которое реально оставляет оформление темы, в каждой теме.
- Этот текст окна намеренно **не** подтягивается в реальном времени со страницы GitHub Releases — это означало бы либо показ сырого английского текста не англоязычным пользователям, либо создание конвейера перевода (отдельный JSON-файл на каждый язык, публикуемый вместе с каждым релизом, или API машинного перевода), которого у этого проекта пока нет. Он остаётся коротким, переведённым вручную резюме, которое переписывается (на месте — всё значение целиком заменяется, никогда не дополняется) при каждом обновлении заметок о релизе для текущей версии.

### Новые/Обновлённые Языковые Ключи (13 языков)

| Ключ | Значение на русском |
|---|---|
| `Lang.Options.Buttons.Update` | Обновить |
| `Lang.Windows.OperationPending.Tasks.Update.Done` | Обновление готово — просмотрите изменения ниже и нажмите Готово. |
| `Lang.Windows.NoUpdateAvailable.Title` | У вас последняя версия |
| `Lang.Windows.NoUpdateAvailable.Text` | Вы используете последнюю версию ModAPI. |
| `Lang.Windows.NoUpdateAvailable.Buttons.OK` | OK |
| `Lang.Options.Labels.UpdateVersionsTableDisabledHint` | Сначала укажите путь к игре, чтобы использовать это. |
| `Lang.Options.Labels.UpdateVersionsTableEnabledHint` | Когда игра обновляется патчем, ModAPI должен распознать новую версию. Включите это, чтобы эта информация обновлялась автоматически. |
| `Lang.Options.Labels.UseSteamEnabledHint` / `UseSteamDisabledHint` | Объясняет, что даёт автоопределение пути к Steam (или ввод вручную) |
| `Lang.Options.Labels.DevLogEnabledHint` / `DevLogDisabledHint` | Объясняет дополнительный файл `ModAPI.dev.log` в сравнении с обычным журналом |
| `Lang.Options.Labels.ClearLogsOnStartEnabledHint` / `ClearLogsOnStartDisabledHint` | Объясняет очистку предыдущего журнала при каждом запуске в сравнении с дополнением к нему |
| `Lang.Options.Labels.AlwaysOnTopEnabledHint` / `AlwaysOnTopDisabledHint` | Объясняет удержание окна поверх других в сравнении с тем, что его могут перекрыть |
| `Lang.Windows.FirstSetup.WhatsNewTitle` | Что нового в этой версии |
| `Lang.Windows.FirstSetup.WhatsNewText` | Полная сводка по разделам (маркеры ■/▸/•) — переписывается на месте при каждом релизе, актуальный текст см. в приложении |
| `Lang.Windows.FirstSetup.Buttons.Close` | Закрыть |
| `Lang.Mods.Welcome.Buttons.OpenWelcomePopup` | Добро пожаловать! |

**Удалены** (нерабочая функция "Автообновление"): `Lang.Options.Labels.AutoUpdate` (заменена на `Lang.Options.Buttons.Update` выше), `Lang.Windows.FirstSetup.AutoUpdate`, `Lang.Windows.FirstSetup.AutoUpdateText`.

**Удалены** (переработка окна первого запуска): `Lang.Windows.FirstSetup.Steam`, `Lang.Windows.FirstSetup.SteamText`, `Lang.Windows.FirstSetup.UpdateVersions`, `Lang.Windows.FirstSetup.UpdateVersionsText`, `Lang.Mods.Welcome.Title0` (заменён на `Lang.Mods.Welcome.Buttons.OpenWelcomePopup`).
...
<details>
<summary><b>Изменения в v2.0.9622</b></summary>

## Изменения в v2.0.9622

### Исправление Ошибки — Расчёт Контрольной Суммы Унифицирован

Проверка целостности в `StartGame()` (Проверка B) самостоятельно пересчитывала контрольную сумму через `FileValidator.ComputeAssemblyChecksum()`, которая всегда хеширует только фиксированную пару файлов (`Assembly-CSharp` + `Assembly-CSharp-firstpass`). Это структурно не подходило для игр вроде The Forest, которые связывают 4 файла (firstpass + основной + UnityScript-firstpass + UnityScript) — проверка сообщала о ложном несоответствии контрольной суммы, даже когда файлы игры оставались нетронутыми.

- `Game.CheckSumGame` (уже корректно вычисляемое `GenerateCheckSums()` в момент `Verify()` по реальному списку `VersionsData.CheckFiles` каждой игры — 2 файла для Green Hell, 4 для The Forest и т.д.) теперь открыто как `public` и напрямую переиспользуется в `StartGame()` вместо пересчёта с другим, жёстко заданным набором файлов.
- Расчёт контрольной суммы теперь унифицирован в единый источник истины (`GenerateCheckSums()`), независимо от того, сколько файлов реально требуется конкретной игре.

### Изменённые файлы

| Файл | Путь | Изменение |
|---|---|---|
| `Game.cs` | `ModAPI_Shared\Data\` | `CheckSumGame` изменено с `protected` на `public` |
| `MainWindow.xaml.cs` | `ModAPI\Windows\` | Проверка целостности в `StartGame()` переиспользует `targetGame.CheckSumGame` вместо пересчёта через `FileValidator.ComputeAssemblyChecksum()` |
...
<details>
<summary><b>Изменения в v2.0.9621</b></summary>

## Изменения в v2.0.9621

### Новые функции

#### Автоопределение по всей библиотеке Steam

Теперь, если игра не найдена через жёстко заданные `SearchPaths`, `FindGamePath()` также ищет во **всех библиотеках Steam, зарегистрированных в системе** (парсится один раз из `libraryfolders.vdf`, кешируется на сессию). Это применяется ко всем 5 поддерживаемым играм, а не только к текущей активной.

- Новый `Game.GetSteamLibraryFolders()` — парсит `libraryfolders.vdf`, статический кеш на сессию
- Управляется флажком **Подключение к Steam**: выключен (по умолчанию при новой установке) → автоопределение полностью пропускается для всех 5 игр, пути остаются пустыми до ручной настройки. Включён → все 5 игр ищутся единообразно тем же способом.

#### Автоматическое обнаружение модов для другой игры

Файл `.mod`, помещённый в папку не той игры (например, мод для Green Hell, скопированный в `mods\TheForest\`), теперь обнаруживается автоматически, вместо того чтобы незаметно ломать операцию Apply.

- `Game.CheckModGameCompatibility()` (используется внутри `ApplyMods()`) перед началом внедрения проверяет, действительно ли каждый тип `AddMethod`/`AddField`/`InjectInto`, объявленный модом, существует в реальных сборках целевой игры. Несовместимые моды автоматически исключаются из этого Apply; остальные применяются как обычно.
- `Game.CheckModGameCompatibilityLight()` + `Game.GetCachedTypeNames()` выполняют ту же проверку в момент загрузки мода (облегчённая версия — читает байты сборки в память, извлекает имена типов, немедленно освобождает файл). Несовместимые моды показывают **значок предупреждения ⚠** с подсказкой на вкладке Mods ещё до нажатия Apply.
- Если моды были исключены и/или в итоге ничего не применено, Start Game показывает одно объединённое всплывающее окно вместо нескольких, наложенных друг на друга; игра не запускается, если не осталось ни одного применённого мода (`Game.LastAppliedModCount`).

#### Вкладка Настройки — Лог разработчика / Очищать логи при запуске

Два новых флажка, после **Подключение к Steam** и перед **Поверх всех окон**:

| Ключ | Описание |
|---|---|
| `Lang.Options.Labels.DevLog` | Включает `ModAPI.dev.log` (переименован из `ModAPI.detailed.log`) — эквивалентно запуску с `--dev` |
| `Lang.Options.Labels.ClearLogsOnStart` | Очищает папку `logs\` при каждом запуске |

`Debug.ClearLogs()` закрывает открытые потоки логов перед удалением файлов, избегая ошибок "файл используется".

#### Глобальное журналирование необработанных исключений

`App.xaml.cs` теперь перехватывает `DispatcherUnhandledException` (поток UI) и `AppDomain.UnhandledException` (фоновые потоки). Исключения, которые раньше приводили к падению приложения без единого следа в логе, теперь фиксируются — тип, сообщение и полный стек вызовов — перед завершением процесса.
...
|---|---|---|---|
| 1 | `Configuration.cs` | `GetPath()` разрешал явно сброшенный (пустая строка) путь в `RootPath` вместо `""`, поскольку `Path.GetFullPath(RootPath + разделитель + "")` сворачивается в `RootPath` | Пустые сохранённые значения теперь сразу возвращают `""`, до объединения пути |
| 2 | `MainWindow.xaml.cs` | Порядок валидации Start Game различался между фильтром "Все" и конкретным фильтром, иногда показывая всплывающее окно выбора мода или игры раньше более фундаментальной проблемы (отсутствие пути Steam/игры) | Оба пути теперь следуют одному порядку: Steam → путь игры → выбор модов → выбор игры |
| 3 | `MainWindow.xaml.cs` | Сбор модов для Start Game игнорировал активный фильтр игры — отмеченные моды другой (невидимой) игры всё равно учитывались, вызывая неверное окно | Сбор модов теперь учитывает текущий фильтр; только "Все" агрегирует все игры |
| 4 | `ModsViewModel.cs` | `Mod.Mods` индексировался только по `{ModId}-{Версия}`, поэтому одинаковые имена файлов в двух разных папках игр конфликтовали — `Load()` второго никогда не вызывался | Ключ теперь включает GameId: `{GameId}-{ModId}-{Версия}` |
| 5 | `ModsViewModel.cs` | После исправления №4 `UpdateMods()` всё ещё группировал записи списка только по ModId, объединяя два одноимённых мода из разных игр в одну запись — сбой с `ArgumentException: An item with the same key has already been added`, когда оба объявляли одну и ту же версию | Группировка отображения теперь также сравнивает GameId |
| 6 | `Game.cs` | Список `<files>` в `Versions.xml` Green Hell содержит одни и те же два файла дважды с разным регистром (`_Data`/`_data`); `CheckFiles` был чувствительным к регистру `HashSet<string>`, поэтому оба хешировались, удваивая вычисленную контрольную сумму и вызывая ложные ошибки целостности | `CheckFiles` теперь использует `StringComparer.OrdinalIgnoreCase` |
| 7 | `Game.cs` / `ModLib.cs` | Шаг "удаление старых файлов" в `ModLib.Create()` не имел защиты повторными попытками от заблокированного `BaseModLib.dll`, а у `Game.CreateModLibrary()` вообще не было обработки исключений — блокировка приводила к падению всего приложения в фоновом потоке | В шаг удаления добавлен цикл повторных попыток 10×500мс; `CreateModLibrary()` теперь оборачивает вызов в try/catch |
| 8 | `MainWindow.xaml.cs` | Когда `ApplyMods()` завершался без фактического применения ни одного мода (например, все исключены), он всё равно сигнализировал завершение как настоящий успех, поэтому игра запускалась без каких-либо модификаций | `Game.LastAppliedModCount` различает "ничего не применено" и "применено N"; запуск пропускается при 0 |
| 9 | `MainWindow.xaml.cs` | Высота окна не пересчитывалась ни при изменении размера шрифта, ни при загрузке при старте сохранённого крупного размера шрифта, ни при переключении на вкладку Настройки (`Tabs_SelectionChanged` была пустой) — при крупных шрифтах нижняя карточка пути игры обрезалась | Пересчёт высоты добавлен во всех трёх местах |
| 10 | `MainWindow.xaml.cs` | У `UpdateWindowHeight()` не было верхнего предела — одновременное разворачивание всех 5 карточек путей игр могло увеличить окно до размера всего экрана или больше | Высота теперь ограничена значением `SystemParameters.WorkArea.Height` |
| 11 | `MainWindow.xaml.cs` | Папки `mods\`/`projects\` создавались безусловно для всех 5 игр при каждом запуске, независимо от того, установлена ли игра | Папки теперь создаются только для игр с проверенным путём и существующим исполняемым файлом |
| 12 | `Game.cs` | `UpdateVersions()` мог не сохранить `Versions.xml`, если целевая папка ещё не существовала (до сих пор было скрыто, поскольку все 5 папок распространяются уже закоммиченными) | Папка создаётся через `Directory.CreateDirectory()` непосредственно перед сохранением |

---

### Вкладка Настройки — Изменены значения по умолчанию при первом запуске

`AutoUpdate`, `UseSteam` (Подключение к Steam) и `UpdateVersionsTable` (Поддерживать таблицу VersionsData) теперь по умолчанию **выключены** при новой установке (ранее были включены по умолчанию). Эти три функции всё ещё не завершены на стороне сервера, поэтому теперь они опциональны — как и `DevLog`/`ClearLogsOnStart`.

### UI

- Строка флажков вкладки Настройки (`SettingsCheckboxes`): `StackPanel` → `WrapPanel`, чтобы подписи переносились на новую строку вместо обрезания при крупных размерах шрифта.
...
| Файл | Путь | Изменение |
|---|---|---|
| `MainWindow.xaml.cs` | `ModAPI\Windows\` | Единый порядок валидации Start Game, учёт фильтра при сборе модов, объединённое всплывающее окно результата, автоопределение 4 игр через библиотеку Steam под управлением UseSteam, исправления высоты окна (размер шрифта / смена вкладки / предел) |
| `MainWindow.xaml` | `ModAPI\Windows\` | Флажки DevLog/ClearLogsOnStart на вкладке Настройки, `WrapPanel` |
| `Game.cs` | `ModAPI_Shared\Data\` | Поиск по библиотеке Steam, регистронезависимый `CheckFiles`, проверки совместимости модов (полная + облегчённая), `LastAppliedModCount`/`LastExcludedModsSummary`, обработка исключений в `CreateModLibrary()`, автоопределение под управлением UseSteam |
| `ModLib.cs` | `ModAPI_Shared\Data\` | Цикл повторных попыток при удалении старых файлов |
| `Mod.cs` | `ModAPI_Shared\Data\` | Поле `GameMismatchReason` |
| `Configuration.cs` | `ModAPI_Shared\Configurations\` | Исправление ошибки пустого пути в `GetPath()` |
| `Debug.cs` | `ModAPI_Shared\` | Переименование в `ModAPI.dev.log`, поле `DevMode`, `ClearLogs()` |
| `App.xaml.cs` | `ModAPI\` | Глобальные обработчики исключений, привязка `Debug.DevMode` |
| `ModsViewModel.cs` | `ModAPI\Data\ViewModels\` | Ключи `Mod.Mods` по игре, группировка отображения по игре, значок несовместимости, подавление спама в логах |
| `ModViewModel.cs` | `ModAPI\Data\ViewModels\` | `HasGameMismatch`/`GameMismatchTooltip` |
| `SettingsViewModel.cs` | `ModAPI\Data\ViewModels\` | `DevLog`/`ClearLogsOnStart`, опциональные значения по умолчанию для 3 существующих флажков |
| `FirstSetup.xaml` | `ModAPI\Windows\SubWindows\` | Значения по умолчанию 3 флажков изменены на выключенные |
| `ModsExcludedWarning.xaml` / `.cs` | `ModAPI\Windows\SubWindows\` | Новый |
| 13x `Language.XX.xaml` | `ModAPI\resources\langs\` | 8 новых ключей |
...
- Автоопределение по всей библиотеке Steam для всех 5 игр, управляется флажком Подключение к Steam
- Автоматическое обнаружение и исключение модов, созданных для другой игры (список + во время Apply), со значком ⚠ на вкладке Mods
- Объединённое всплывающее окно результата для исключённых модов / отсутствия применённых модов вместо наложенных друг на друга окон; игра больше не запускается при нуле применённых модов
- Глобальное журналирование необработанных исключений (поток UI + фоновые потоки)
- `ModAPI.dev.log` заменяет `ModAPI.detailed.log`; новые переключатели на вкладке Настройки для Лога разработчика и Очистки логов при запуске
- `AutoUpdate`/`UseSteam`/`UpdateVersionsTable` теперь по умолчанию выключены при новой установке
- Исправлено: ошибка пустого пути в `Configuration.GetPath()`, непоследовательный порядок валидации Start Game, сбор модов, игнорировавший фильтр, конфликты ключей `Mod.Mods` между играми и вызванный ими сбой `UpdateMods()`, удвоение контрольной суммы Green Hell (`_Data`/`_data`), сбой из-за блокировки файла `BaseModLib.dll`, безусловное создание `mods\`/`projects\`, сбой сохранения `Versions.xml` при отсутствующей папке, отсутствие пересчёта высоты окна при изменении размера шрифта / смене вкладки, неограниченная высота окна при "Развернуть всё"
